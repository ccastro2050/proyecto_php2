...
[6_contracts.md](../docs/spec_kit/versiones/v2_mas_tablas/6_contracts.md)).
**Postman cumple ese papel**: aquí está la colección con los seis recursos de
la v2 ya armados, para verlos y probarlos con clics.
...
2. **Import** (botón arriba a la izquierda) → arrastre el archivo
   `coleccion_v2.postman_collection.json` de esta carpeta.
...
La colección usa `base` = `http://localhost:8026` (el proyecto del curso). Si
está probando **SU reconstrucción** (la de la
[GUIA_IA2](../docs/spec_kit/versiones/v2_mas_tablas/GUIA_IA2.md), que corre
con los puertos +100): clic en la colección → pestaña **Variables** → cambie
`base` a `http://localhost:8126`. Una sola edición y todas las peticiones
