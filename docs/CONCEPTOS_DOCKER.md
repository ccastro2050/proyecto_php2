...
| `EXPOSE 8026` | **Documenta** en qué puerto escucha el programa | No abre nada: quien publica el puerto es el `ports:` del compose |
| `CMD ["php", "-S", "0.0.0.0:8026", "index.ph…` | **El comando que se ejecuta al encender** el contenedor | Si ese proceso termina, el contenedor se apaga |
...
flowchart LR
    ...
    subgraph PC["Su PC — Docker Desktop (el 'centro de datos')"]
        subgraph RED["red interna del compose (LAN virtual, con DNS propio)"]
            FRONTPHP["SERVIDOR WEB (front)<br/>contenedor front-php<br/>hostname: front-php · escucha en 8024"]
            APIFACTURAS["SERVIDOR DE APLICACIONES<br/>contenedor api-facturas<br/>hostname: api-facturas · escucha en 8026"]
    ...
    NAV -->|"localhost:8026"| APIFACTURAS
    NAV -->|"localhost:8024"| FRONTPHP
    NAV -->|"localhost:8102"| PHPMYADMIN
    APIFACTURAS -->|"mariadb:3306 (DNS de Docker)"| MARIADB
    FRONTPHP -->|"http://api-facturas:8026<br/>al controlador, por el NOMBRE"| APIFACTURAS
    PHPMYADMIN -->|"mariadb:3306 (DNS de Docker)"| MARIADB
    CLIENTE -.->|"opcional (diagnóstico):<br/>localhost:13327"| MARIADB
```
...
docker network create proyecto_php2_default

# 2. mariadb
docker run -d --name mariadb --network proyecto_php2_default --restart unless-stopped `
...
docker run -d --name api-facturas --network proyecto_php2_default --restart unless-stopped `
  ...
  -p 8026:8026 api-facturas

# 5. Construir la imagen de front-php y encenderla
docker build -t front-php ./front_php
docker run -d --name front-php --network proyecto_php2_default --restart unless-stopped `
  -e "URL_API=http://api-facturas:8026" `
  -v "${PWD}/front_php:/app" `
  -p 8024:8024 front-php

# 6. phpmyadmin
docker run -d --name phpmyadmin --network proyecto_php2_default --restart unless-stopped `
  -e "PMA_HOST=mariadb" `
  -e "PMA_USER=paradigmas" `
  -e "PMA_PASSWORD=paradigmas123" `
  -p 8102:80 phpmyadmin:latest
...
```yaml
URL_API: http://api-facturas:8026
```
...
|---|---|---|
| **Su navegador** | `http://localhost:8026` | Está fuera de Docker. Usa el puerto **publicado** |
| **Otro contenedor** | `http://api-facturas:8026` | Están en la misma red: se llaman por el **nombre del servicio** |
...
> **En este proyecto los dos números son iguales** (`8026:8026`), y eso
...
Si alguien cambiara esa línea por `http://localhost:8026`, el front
...
```powershell
docker network inspect proyecto_php2_default
```
...
`docker-compose.yml` y le pega `_default`. La carpeta `proyecto_php2` produce
la red `proyecto_php2_default`. Tres formas de confirmarlo:
...
docker inspect proyecto_php2-front-php-1 --format '{{range $k,$v := .NetworkSettings.Networks}}{{$k}}{{end}}'
```
...
network proyecto_php2_default has active endpoints (name:"proyecto_php2-front-php-1" …)
```
