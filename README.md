# Proyecto PHP — versión 2: seis recursos, integridad y facturas
...
> el puerto 8026 (pasa al reiniciar el PC: la API vieja revive sin su
...
> ```
> NAMES                          STATUS                    PORTS
> proyecto_php2-api-facturas-1   Up 2 hours                0.0.0.0:8026->8026/tcp
> proyecto_php2-mariadb-1        Up 2 hours (healthy)      0.0.0.0:13327->3306/tcp
> ```
>
...
```powershell
git clone https://github.com/ccastro2050/proyecto_php2.git
cd proyecto_php2
...
| Qué | Dónde |
|---|---|
| **La pantalla** — empiece por aquí | **http://localhost:8024** |
| **API Facturas** — diagnóstico | http://localhost:8026/ |
| Listar facturas (JSON) | http://localhost:8026/api/factura |
| **phpMyAdmin** (administrar MariaDB desde el navegador) | http://localhost:8102 |
| MariaDB (para SQLTools/DBeaver, opcional) | `localhost:13327` · `paradigmas`/`paradigmas123` |

> **Los puertos son distintos a los de la v1** a propósito: así se pueden
> tener las dos versiones encendidas al mismo tiempo y compararlas.

**Empiece por el 8024**, que es como lo ve alguien que no programó esto. Hay
tres cosas que vale la pena probar de una, porque son lo que esta versión
viene a enseñar:

1. **Vaya a Personas e intente eliminar a Ana Torres.** No se deja, y la
   pantalla explica por qué en español: ya es cliente. Esa negativa no la
   escribió ningún `if` de PHP — la puso la base de datos, y la API solo la
   tradujo.
2. **Cree una factura con dos productos.** Al guardar verá el total. **Ese
   número no lo escribió nadie**: lo calculó un trigger. Vuelva a Productos y
   compruebe que el stock bajó.
3. **Anule esa factura.** Sigue existiendo, marcada, y el stock volvió.
   Anular no es borrar, y un negocio no borra facturas.
...
Refresque <http://localhost:8024/facturas>: la pantalla sigue en pie, con su
...
> ℹ️ Este proyecto usa los puertos 8024, 8026, 8102 y 13327: si alguno ya
...
> 🤖 ¿Va a trabajar con IA? Siga la **[Guía para construir la versión 2 con
> IA](docs/spec_kit/versiones/v2_mas_tablas/GUIA_IA2.md)** — cubre los dos
...
│
├── api_facturas/                # LA API — PHP puro, sin framework (puerto 8026)
...
│
├── front_php/                   # LA PANTALLA — PHP puro también (puerto 8024)
...
v2  seis recursos: + empresa, persona, cliente,  ← USTED ESTÁ AQUÍ
    vendedor y factura (maestro-detalle)           (cerrada: tag v2)
v3  segundo motor (PostgreSQL) — nace la fábrica  repo proyecto_php3
...
| [2_spec.md](docs/spec_kit/versiones/v2_mas_tablas/2_spec.md) | QUÉ construir y los 14 criterios de aceptación |
| [3_plan.md](docs/spec_kit/versiones/v2_mas_tablas/3_plan.md) | CÓMO: estructura, capas y las piezas nuevas |
...
| [5_data_model.md](docs/spec_kit/versiones/v2_mas_tablas/5_data_model.md) | Las seis tablas, sus relaciones y **quién escribe cada columna** |
| [6_contracts.md](docs/spec_kit/versiones/v2_mas_tablas/6_contracts.md) | Los endpoints de los seis recursos y las pantallas, con formatos exactos |
| [7_quickstart.md](docs/spec_kit/versiones/v2_mas_tablas/7_quickstart.md) | Smoke test para validar lo construido |
